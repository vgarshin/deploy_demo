...
def train_model():
    ...
    TFIDF_VECTORIZERS.update({model_id: tfidf_vectorizer})
    print('list tf-idf vectorizers updated')
    print('classes labels:', lb.classes_)
    CLASSES.update({model_id: lb.classes_})
    ...
    PREDICTORS.update({model_id: estimators})
    model_description = {model_id: metrics}
    METRICS.update(model_description)
    print('predictors list updated, models in list: ', PREDICTORS.keys())
    print('...train for model {} completed'.format(model_id))
    return resp(200, model_description)
